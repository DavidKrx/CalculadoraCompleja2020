...
public class VerComplejo extends Application {
	...
	private void StratBinding() {
		...
		resultImagineTxt.textProperty().bindBidirectional(complejoResul.imaginarioProperty(), new NumberStringConverter());
		
		realTxt1.textProperty().addListener((obj, o, n) -> { this.calcula(); });
		imagineTxt1.textProperty().addListener((obj, o, n) -> { this.calcula(); });
		
		realTxt2.textProperty().addListener((obj, o, n) -> { this.calcula(); });
		imagineTxt2.textProperty().addListener((obj, o, n) -> { this.calcula(); });
		
		operaciones.valueProperty().addListener((obj, o, n) -> { this.calcula(); });
		
	}
	
	public void calcula() {
		System.out.println(operaciones.getSelectionModel().getSelectedItem());
